...
| 2026-08-22 | 1.1.0 | master 최신 기준으로 이식하며 프론트엔드 요구사항 명문화 (REQ-EIA-016~019). 원안은 제외 범위에 "프론트엔드 UI 없음"이라 적었으나 실제 구현(PR #5)은 페이지·훅·라우트·사이드바 항목을 함께 담고 있어 문서와 어긋나 있었다 — 해당 제외 항목을 삭제하고 구현된 동작을 요구사항으로 기록했다. 사이드바 항목 위치는 SPEC-NAV-SIDEBAR-001 v1.1.0의 그룹 구조에 맞춰 "도서관리" 그룹에 배치한다 |
...
**REQ-EIA-016** (Ubiquitous)
The 시스템 shall `/books/etoile-add-isbn` 경로에 ISBN을 줄 단위로 입력하는 페이지를 제공한다. 입력값은 줄바꿈으로 분리하고 각 항목의 앞뒤 공백을 제거한 뒤 빈 줄을 제외하여 `skus` 배열로 전송한다.

**REQ-EIA-017** (Event-Driven)
When API 응답이 수신될 때, the 시스템 shall REQ-EIA-013의 네 범주를 각각 건수와 SKU 목록으로 구분하여 표시한다 — 본관 신규 생성 후 Etoile 등록(`status=-1`), 본관 기존 → Etoile 등록(`status=0`), 이미 Etoile에 등록되어 건너뜀.

**REQ-EIA-018** (Ubiquitous)
The 시스템 shall 사이드바 "도서관리" 그룹에 `/books/etoile-add-isbn` 로 연결되는 "Etoile ISBN 추가" 항목을 표시한다 (SPEC-NAV-SIDEBAR-001 v1.1.0의 그룹 구조 기준).

**REQ-EIA-019** (Unwanted)
If API 요청이 실패하면, then the 시스템 shall 오류 메시지를 표시하고 페이지를 크래시시키지 않는다.
...
### Inven (본관 재고)
- 테이블: `book_inven`
- 주요 필드: `inven_SKU` (`db_index=True`, **유니크 제약 없음**), `vendor`, `store`, `is_prepared`, `status_of_shopify`, `is_use`
  - v1.1.0 정정: 원안은 `inven_SKU`를 PK로 기술했으나 실제 PK는 `id`이고 `inven_SKU`에는 유니크 제약이 없다. 구현은 SKU→Inven 매핑을 dict로 만들므로 같은 SKU의 Inven 행이 둘 이상이면 그중 하나에만 Etoile 레코드가 붙는다. 이 동작은 기존 `InvenSkuBulkAddView`(SPEC-INVEN-ADD-001)의 가정과 동일하며, SKU 중복 행 자체를 정리하는 것은 이 SPEC의 범위 밖이다.
