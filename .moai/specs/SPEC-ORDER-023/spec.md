...
6. **발주상태는 미발주/발주완료 2값뿐이다**: trackable 라인아이템 중 하나라도 `purchase_status == 'unordered'` → **미발주**; 그 외(trackable이 1개 이상 존재) → **발주완료**; trackable 0개 → `-`. Order 레벨 집계 컬럼은 신설하지 않는다(표시 전용, 매 요청 시 이미 prefetch된 라인아이템에서 계산).
...
**REQ-OLIST-013** (State-Driven): While an order has at least one trackable line item and any trackable line item's `purchase_status` equals `"unordered"`, the system shall set that order's `purchase_display` to `"unordered"`.

**REQ-OLIST-014** (State-Driven): While an order has at least one trackable line item and no trackable line item's `purchase_status` equals `"unordered"`, the system shall set `purchase_display` to `"ordered"` — this includes orders whose trackable line items are all `purchase_status == "order_cancelled"` (명시적 가정 4).
...
**REQ-OLIST-021** (Ubiquitous) [HARD]: `logistics_display` and `purchase_display` shall be computed only from `LineItem` rows already available via `OrderListView`'s existing `prefetch_related("line_items")` cache (`backend/order/views.py:162`), without issuing any additional database query.
...
**AC-OLIST-014** (State-Driven) [HARD] — 미발주 판별(`any` 대 `all` mutation). Traces: REQ-OLIST-013. **While** an order has two trackable line items — A(`purchase_status='unordered'`), B(`purchase_status='in_stock'`) — the system **shall** set `purchase_display` to `"unordered"`.
*Mutation*: 조건을 "모든 trackable 항목이 unordered"로 구현하면(all) A/B가 섞여 있으므로 거짓이 되어 `"ordered"`(오답)를 반환한다.

**AC-OLIST-015** (State-Driven) — 발주완료 판별. Traces: REQ-OLIST-014. **While** an order has two trackable line items, neither with `purchase_status='unordered'`(예: `in_stock`, `cs_required`), the system **shall** set `purchase_display` to `"ordered"`.
...
**AC-OLIST-021** (Ubiquitous) [HARD] [C1 재설계 — 절대상수를 SPEC 자체가 유도한다] — 전체 쿼리 수는 SPEC이 유도한 값과 정확히 같다. Traces: REQ-OLIST-021, REQ-OLIST-022, REQ-OLIST-022a. **While** a page contains 1 order versus 5 orders(둘 다 확정 매입가·유효 환율·고객 있는 trackable 라인아이템), the system **shall** issue the same total query count for both requests, **and** that count **shall equal exactly 7**(REQ-OLIST-022a가 유도한 값: 이 SPEC 시작 시점에 이 세션에서 직접 측정한 베이스라인 6 + `ExchangeRate` 배치 쿼리 1 — 아래 계산 근거 참조).
...
| REQ-OLIST-013 | AC-OLIST-011, AC-OLIST-014 |
| REQ-OLIST-014 | AC-OLIST-015 |
