...
| 1.5.0 | 2026-08-17 | ggajo | **환불 차감 + 주문상세/주문목록 표시 일원화.** (1) 신규 REQ-OLIST-008a: trackable 정의에 "전량 환불되지 않았을 것"을 추가한다 — 사용자 보고 주문 #37830이 19개 품목 중 18개 출고 완료인데 부분출고로 표시됐고, 원인은 전량 환불된 19번째 품목이 `not_shipped`로 남아 규칙 1을 깬 것이었다. 그 품목의 `purchase_status`는 `cs_required`였다 — 취소 신호는 Refund 행에 있으므로 `order_cancelled` 검사로는 잡을 수 없다. 표시 경로(`_derive_line_item_states`)와 SQL 필터 경로(`_apply_logistics_display_filter`)에 동일하게 적용 — REQ-OLIST-025의 두 경로 동치성 제약 그대로다. 프로덕션 영향: 부분출고→전체출고 12건, 혼재→전체출고 3건, 미발주→발주완료 4건, 전량 취소 주문 22건은 표시값 없음(`-`). (2) 신규 REQ-OLIST-012a: 주문상세도 같은 파생값을 노출한다. 파생 로직을 `LineItemStateDerivationMixin`으로 추출해 목록·상세 serializer가 공유하고, 프론트 라벨 맵도 `features/order/logisticsDisplayLabels.ts`로 추출해 두 화면이 공유한다(기존엔 화면마다 따로 정의돼 `partial`을 서로 다르게 불렀다). 저장 컬럼 낡음으로 인한 508건 불일치가 해소된다. `OrderDetailView`에 `line_items__purchase_orders` prefetch 추가 — 라인아이템 수만큼의 쿼리를 상수 1개로 대체하며, 그 결과 SPEC-ORDER-021의 `ORDER_DETAIL_QUERY_COUNT`가 7→8이 된다(해당 SPEC v1.5.0에 기록). 목록 쿼리 수 8은 불변. **테스트**: `test_spec_023.py` 41개 통과(신규 8건 — 환불 차감 5건, 상세 일원화 3건), 프론트 309개 통과. |
...
**REQ-OLIST-008a** (Ubiquitous) `[NEW v1.5.0]`: The system shall treat a line item as **trackable** for REQ-OLIST-008 through REQ-OLIST-015 only when its `sku` is not null AND its ordered quantity is not fully refunded (`quantity - Σrefunded > 0`). A fully refunded line item is as cancelled as `purchase_status="order_cancelled"` — the customer is not owed those copies — and takes no part in either `logistics_display` or `purchase_display`; a PARTIAL refund leaves stock still owed and is not an exclusion. This is the same net-quantity rule `UnorderedItemsView` and `_fully_refunded_line_item_ids` already apply. Both the Python display path and the SQL filter path (REQ-OLIST-025) must implement it identically.
...
**REQ-OLIST-012a** (Ubiquitous) `[NEW v1.5.0]`: The system shall expose `logistics_display` and `purchase_display` on the order DETAIL response as well, derived by the identical shared rule (REQ-OLIST-008~012) — never from the stored `Order.status` column. The detail screen previously rendered that stored aggregate, which only changes when `purchase_order_views._recompute_order_aggregates()` runs; neither outbound path calls it, so 508 of 3,693 production orders showed the list one value and the detail another. The stored `status` / `ready_to_ship` fields remain in the detail payload for their other consumers.
