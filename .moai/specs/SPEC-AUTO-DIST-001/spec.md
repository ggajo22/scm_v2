...
| 1.1.0 | 2026-06-23 | ggajo | auto_select_distributor 호출 시점 변경 — UploadVendorFileView에서 제거, 별도 RunComparisonView로 이관; RunComparisonView는 미발주 LineItem 기준으로 SKU별 수량 집계 후 자동 선택 실행 |
...
- `backend/order/purchase_order_views.py` — `UploadVendorFileView`에서 auto_select 제거, `RunComparisonView` 신규 추가, comparison API 응답 필드 확장
- `backend/order/urls.py` — `POST /api/purchase-orders/run-comparison/` 등록
...
### UploadVendorFileView 수정 (v1.1.0 변경)

`UploadVendorFileView`는 Excel 파싱 후 `VendorComparison` upsert만 수행한다.
`auto_select_distributor()` 호출은 이 뷰에서 **제거**되었다.

### RunComparisonView (v1.1.0 신규)

`POST /api/purchase-orders/run-comparison/` 에서 `auto_select_distributor()`를 실행한다.

실행 흐름:
1. 미발주 `LineItem`(`purchase_status="unordered"`, `PurchaseOrder` 미연결)을 조회
2. SKU별로 `total_qty`(환불 차감 후 순수량 합계) 및 `line_items`(id, order_name, quantity) 집계
3. 해당 SKU들의 `VendorComparison` 및 `WarehouseStock`을 일괄 조회 (N+1 방지)
4. 각 SKU에 대해 `auto_select_distributor()` 실행
5. 결과(`selected_distributor`, `candidate_basis`, `price_diff`, `price_diff_alert`)를 `VendorComparison`에 저장
6. SKU별 비교 결과 + 매칭된 `line_items` 목록을 응답으로 반환

`VendorComparison` 레코드가 없는 SKU(업로드 데이터 없음)도 결과에 포함되며, 비교 필드는 모두 `null`로 반환된다.
