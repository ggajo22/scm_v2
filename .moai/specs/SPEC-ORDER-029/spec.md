...
| 0.6.0 | 2026-08-20 | ggajo | **[본 개정 — 위임 실행(run phase 0.7), 신규 감사 없이 적용]** `backfill_order_cancellations` 커맨드의 전체 실패 경로(store-level, chunk-level, credentials)를 테스트하는 신규 AC 3건(AC-CANC-026/027/028)을 acceptance.md에 추가했다. 실측(2026-08-19): 백필 1회 완전 실행 — 3,919건 조회, 3,094건 변경(52건 취소 + 3,042건 종료), chunk 실패 0건, 소요 27초, exit 0. 멱등성 확인(즉시 재실행) — changed=0. 배포 후 감지 커맨드(5분 주기): 후보 집합 gimssine 2,188건 + etoile 47건 = 2,235건 → 10회 API 호출/사이클. 스케줄러(Windows Task Scheduler): `sync_order_cancellations` 작업 등록(5min 주기, `:X1:30` 정렬), `backfill_order_cancellations` 작업 등록(주간 일요일 04:02:45), 스태거 확인(sync_orders `:X4:05` 대비 2m33~2m27초 전후). 첫 예약 실행: 2026-08-19 23:26:32, exit 0, LastTaskResult 0. 야간 실행 확인(2026-08-20 08:21) — exit 0. AC-CANC-026/027/028 세 건 전부 통과. 변이 M26/M27/M28 신규 추가(총 28개 변이, 단독 판별자 유지). REQ 27개 → 27개(불변), AC 25개 → **28개**. |
...
| REQ-CANC-017 | AC-CANC-017, AC-CANC-026, AC-CANC-027 |
| REQ-CANC-018 | AC-CANC-020, AC-CANC-023, AC-CANC-026, AC-CANC-027, AC-CANC-028 |
