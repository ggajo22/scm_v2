...
감지(N=30일 창):
  ...
  상한: 백필과 동일한 무제한 모집단(3,904건)을 절대 넘지 않는다 — 창이 걸린 부분집합이므로.
  참고치: 현재 open 주문만(776+25=801건) 기준으로는 gimssine ⌈776/250⌉=4 + etoile ⌈25/250⌉=1 = 5회.
  실제 값은 "801 + 최근 30일 내 종료 건수"이며, 후자는 배포 후 커맨드 stdout의 candidates=N 출력으로
  직접 관측하는 것을 권장한다(사전 추정치를 확정치처럼 제시하지 않는다).
```
