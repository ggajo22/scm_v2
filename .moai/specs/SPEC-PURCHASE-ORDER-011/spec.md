...
| 1.6.0 | 2026-08-17 | ggajo | **REQ-DEX-009c/009d 폐기(superseded).** 사용자 지시(2026-08-17)로 `_recompute_order_aggregates`의 `ready_to_ship` 규칙에서 `damaged_exchange` 단락과 `cs_required` 단락이 함께 삭제됐다 — 남는 조건은 "비취소·비환불 LineItem 전원이 `logistics_status=received` 또는 `purchase_status=in_stock`" 하나뿐이다(SPEC-ORDER-012 v1.5.0 REQ-RTS-002 참조). 따라서 파손 접수는 더 이상 `ready_to_ship`을 False로 떨어뜨리지 않으며, 입고까지 끝난 파손 품목은 출고가능으로 남는다. REQ-DEX-009c/009d는 이 SPEC의 다른 요구사항(접수·노트 생성·수량 기준·검색 범위)에는 영향을 주지 않는다. 결정 E(공유 함수 fan-in 확장)와 REQ-DEX-013 백필 마이그레이션 0040은 이미 적용된 이력이므로 되돌리지 않는다 — 다만 그 마이그레이션이 기록한 값은 2026-08-17 전수 백필로 새 규칙에 맞게 재계산됐다. **테스트**: `test_valid_submission_sets_status_and_quantity_without_touching_ready_to_ship`(구 `..._flips_ready_to_ship`, 기대값 False→True), `test_damaged_exchange_no_longer_short_circuits_ready_to_ship`(구 `test_ready_to_ship_short_circuit_unit_level`, 기대값 반전)로 갱신, `test_spec_purchase_order_011.py` 전량 통과. |
| 1.7.0 | 2026-08-20 | ggajo | **REQ-DEX-011 반전(사용자 지시).** 파손 접수가 `logistics_status`를 건드리지 않던 규칙을 폐기하고, 접수 시 `logistics_status="not_shipped"`(미입고)로 되돌리며 `received_quantity`에서 파손 수량을 차감하도록 변경했다. v1.6.0에서 `ready_to_ship` 단락(REQ-DEX-009c)이 삭제되면서 입고까지 끝난 파손 품목이 출고가능으로 남던 문제가, 이번에는 특수 분기가 아니라 데이터 사실(미입고)로 자연히 해소된다 — `_recompute_order_aggregates`는 손대지 않았다. **`received_quantity` 동반 차감이 필수인 이유**: `_process_warehouse_receipt_rows`의 적격 게이트는 `logistics_status IN ("not_shipped","shipment_confirmed")`이고 그 다음 `received_quantity + 입고건수 > quantity`면 `quantity_exceeded`로 거부한다 — 상태만 되돌리면 교환본이 도착해도 업로드가 영구 거부되어 그 행은 다시 입고로 돌아갈 수 없다(AC-DEX-011c가 이 경로를 직접 검증). 재접수는 REQ-DEX-009b의 덮어쓰기 시맨틱을 `received_quantity`까지 확장해, 직전 차감분을 복원한 뒤 새 값을 차감한다(3→2 정정 시 1 복원, REQ-DEX-011a/AC-DEX-011b). 결과는 `[0, quantity]`로 클램프해 결정 G 이전에 생성된 `damaged_quantity=0` 레거시 행도 범위를 벗어나지 못하게 했다. 신규 REQ-DEX-011a(재접수 복원)·REQ-DEX-011b(구 011의 `shipped_quantity` 불변 절을 분리, `shipped_at`/`received_at` 추가)·REQ-DEX-013b(1회성 백필 마이그레이션 `0045_backfill_damaged_exchange_logistics`, 선례 0040) 추가. AC-DEX-011 재작성 + 신규 AC-DEX-011a/011b/011c/013b. **기존 데이터**: 원격 DB 직접 조회 결과 `damaged_exchange` 행은 정확히 1건(LineItem 17142, `quantity=1, damaged_quantity=1, logistics_status=received, received_quantity=1, shipped_quantity=0`)으로 정확히 이 마이그레이션이 대상으로 하는 형태였다. 마이그레이션 0040은 삭제된 단락 규칙을 쓰므로 재사용하지 않고, 0045는 현행 규칙(환불 차감 포함, 단락 없음)을 재구현한다. **테스트**: `test_valid_submission_resets_to_not_shipped_and_flips_ready_to_ship`(구 `..._without_touching_ready_to_ship`, 기대값 반전)·`test_shipped_quantity_untouched_by_the_not_shipped_reset`(구 `test_logistics_status_and_shipped_quantity_untouched`) 갱신, 신규 4건 + `TestBackfillDamagedExchangeLogisticsMigration` 6건 추가. REQ: 32개 → 35개 항목. AC: 38개 → 43개 항목. |
| 1.7.1 | 2026-08-22 | ggajo | **표시 라벨 축약 정합 (문서 전용).** 파손 교환 테이블의 헤더 문자열이 `오늘 출고 가능 여부` → `오늘출고` 로 축약됐다 — 긴 헤더가 컬럼 폭을 결정하고 있었다. REQ-DEX-006에 개념명과 렌더링 문자열의 구분을 명시했다. 요구사항·동작·테스트 변경 없음(세 가지 상태 표시 규칙과 AC-DEX-005a는 그대로). |
...
4. 행(row) 단위로 파손 수량을 입력해 접수하면 `purchase_status`를 `damaged_exchange`로 전환하고, 신규 필드 `damaged_quantity`에 접수 수량을 기록하며(재접수 시 덮어쓰기), 해당 품목에 대한 메모(`LineItemNote`, `note_type="파손/교환"`, `assignee="발주"`, `author`=접수자)를 자동 생성하고, 부모 Order 집계를 기존 관례대로 재계산한다. **(v1.7.0 개정)** 접수는 `logistics_status`를 `not_shipped`로 되돌리고 `received_quantity`에서 파손 수량을 차감한다 — `shipped_quantity`/`shipped_at`/`received_at`은 변경하지 않는다.
...
> 표기 주의 (v1.7.1): 이 SPEC 전반에서 쓰는 "오늘 출고 가능 여부"는 `Order.ready_to_ship` 을 가리키는 **개념명**이다. 화면에 렌더링되는 테이블 헤더 문자열은 `오늘출고` 로 축약돼 있다 — 긴 헤더가 컬럼 폭을 결정하고 있었기 때문이다. 세 가지 상태(true/false/null)를 그대로 표시한다는 규칙 자체는 변하지 않는다.
...
**REQ-DEX-009c** (State-Driven) `[MODIFY]` — **SUPERSEDED v1.6.0 (사용자 지시로 폐기, 구현 없음): the damaged_exchange short-circuit was removed from `_recompute_order_aggregates` together with the `cs_required` one; see SPEC-ORDER-012 v1.5.0 REQ-RTS-002. 아래 원문은 이력 보존용이다.**
...
**REQ-DEX-009d** (Unwanted) — **SUPERSEDED v1.6.0 (REQ-DEX-009c 폐기에 종속).** If the REQ-DEX-009c short-circuit evaluates for an Order, then the system shall continue to compute that Order's `status` field using the existing, unmodified `logistics_status` aggregate rule — the short-circuit changes only the `ready_to_ship` output value; it does not alter how `status` is derived (v1.3.0 D16 — "the system shall not modify `status`" is not literally true, since `status` and `ready_to_ship` are always persisted together by the same underlying write; what does not change is the *rule*, see 결정 E).
...
**REQ-DEX-011** (Event-Driven) `[MODIFY v1.7.0]`: When a damage submission (REQ-DEX-009) succeeds for a LineItem, the system shall set that LineItem's `logistics_status` to `"not_shipped"` and reduce its `received_quantity` by the submitted damage quantity, floored at `0` and capped at `quantity` — a damaged copy is not usable stock, so the line re-enters the logistics pipeline at 미입고 (사용자 지시 2026-08-20).

**REQ-DEX-011a** (Event-Driven) `[NEW v1.7.0]`: When a damage submission (REQ-DEX-011) is processed for a LineItem whose `purchase_status` is already `"damaged_exchange"`, the system shall add the previously submitted `damaged_quantity` back to `received_quantity` before subtracting the new value, so a corrected resubmission does not subtract twice — the same overwrite-not-accumulate semantics REQ-DEX-009b defines for `damaged_quantity` itself.

**REQ-DEX-011b** (Unwanted) `[MODIFY v1.7.0]`: If a damage submission (REQ-DEX-009) is processed for a LineItem, then the system shall NOT modify that LineItem's `shipped_quantity`, `shipped_at`, or `received_at`.
...
**REQ-DEX-013b** (Event-Driven) `[NEW v1.7.0]`: When the REQ-DEX-011 logistics backfill migration is applied, the system shall, for every existing LineItem with `purchase_status == "damaged_exchange"`, set `logistics_status="not_shipped"` and `received_quantity = max(0, received_quantity - damaged_quantity)`, and then recompute `Order.status` and `Order.ready_to_ship` for exactly those parent Orders under the current aggregate rule (refund netting included, no cs_required/damaged_exchange short-circuit) — leaving every other Order untouched.
...
**AC-DEX-011** (Event-Driven) `[MODIFY v1.7.0]` — Traces: REQ-DEX-011, REQ-DEX-011b. When a damage submission of `4` succeeds for a LineItem with `quantity=8`, `logistics_status="outbound_scheduled"`, `received_quantity=8`, and `shipped_quantity=2`, the system shall set `logistics_status="not_shipped"` and `received_quantity=4` while leaving `shipped_quantity=2` and `shipped_at=null` exactly as they were (`test_spec_purchase_order_011.py::TestDamagedExchangeSubmit::test_shipped_quantity_untouched_by_the_not_shipped_reset`).

**AC-DEX-011a** (Event-Driven) `[NEW v1.7.0]` — Traces: REQ-DEX-011. When a damage submission of `3` succeeds for a LineItem with `quantity=5` and `received_quantity=0`, the system shall set `logistics_status="not_shipped"` and leave `received_quantity` at `0` rather than `-3` (`..._test_received_quantity_floors_at_zero`).

**AC-DEX-011b** (Event-Driven) `[NEW v1.7.0]` — Traces: REQ-DEX-011a. When a LineItem with `quantity=5` and `received_quantity=5` receives a submission of `3` (leaving `received_quantity=2`) and then a corrected submission of `2`, the system shall set `received_quantity=3` — not `0` — confirming the previously applied subtraction is restored rather than compounded (`..._test_resubmission_restores_over_subtracted_receipt`).

**AC-DEX-011c** (Event-Driven) `[NEW v1.7.0]` — Traces: REQ-DEX-011. When a warehouse-receipt upload row is processed for a LineItem with `quantity=1` whose full receipt (`received_quantity=1`, `logistics_status="received"`) was subsequently damage-submitted with `1`, the system shall accept that row — reporting `quantity_exceeded_count=0` and returning the LineItem to `received_quantity=1`, `logistics_status="received"` — rather than rejecting it under `_process_warehouse_receipt_rows`' `received_quantity + count > quantity` guard. This is the discriminating criterion for REQ-DEX-011's `received_quantity` clause: resetting `logistics_status` alone would leave the row permanently stuck at 미입고 (`..._test_replacement_receipt_upload_is_accepted_after_reset`).
...
**AC-DEX-013b** (Event-Driven) `[NEW v1.7.0]` — Traces: REQ-DEX-013b. When the logistics backfill migration runs against a LineItem with `purchase_status="damaged_exchange"`, `quantity=1`, `damaged_quantity=1`, `logistics_status="received"`, and `received_quantity=1` (the sole production row, LineItem 17142) whose Order is stored as `ready_to_ship=True`/`status="received"`, the system shall leave that LineItem at `logistics_status="not_shipped"`/`received_quantity=0` and its Order at `ready_to_ship=False`/`status="not_shipped"`, while an Order in the same run with no `damaged_exchange` LineItem keeps both stored values unchanged (`test_spec_purchase_order_011.py::TestBackfillDamagedExchangeLogisticsMigration`).
