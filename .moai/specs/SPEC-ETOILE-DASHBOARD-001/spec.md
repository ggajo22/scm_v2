...
Etoile 재고(`etoile_book_inven`) 상태를 한눈에 파악할 수 있는 전용 화면이 없어,
운영자가 Etoile Shopify 리스팅 현황을 확인하려면 별도 DB 쿼리가 필요하다.
...
The system shall display an "Etoile 현황" sub-item in the "도서관리" sidebar group, positioned after "빠른 리스팅".
...
When the current route is `/books/etoile`, the system shall apply the active visual style to the "Etoile 현황" sidebar sub-item only.
...
- **Etoile 북인포 데이터 없음**: `etoile_book_info` 테이블 데이터는 포함하지 않음.
...
| `frontend/src/components/Sidebar.tsx` | "Etoile 현황" sub-item 추가 |
