...
| 1.3.0 | 2026-06-23 | ggajo | 업체 데이터 테이블 분리 — BookseenData/KyoboData 독립 테이블 도입; 교보 업로드 시 북센 데이터 소실 버그 수정; VendorComparison은 비교 결과(selected_distributor 등)만 저장; UploadVendorResponse 타입 오류 수정(processed_count→parsed_count) |
...
**When** 관리자가 `POST /api/purchase-orders/upload-vendor-file/`에 Excel 파일을 업로드하면, the 시스템 **shall** 파일을 파싱하여 SKU별 재고 여부와 단가를 배급사에 따라 `BookseenData`(bookseen) 또는 `KyoboData`(kyobo) 레코드에 독립적으로 저장한다. 두 테이블은 완전히 분리되어 있어 한 배급사의 업로드가 다른 배급사의 데이터에 영향을 주지 않는다.
...
**When** 관리자가 `POST /api/purchase-orders/run-comparison/`을 호출하면, the 시스템 **shall** 현재 미발주 상태(`purchase_status="unordered"`)인 `LineItem`을 SKU별로 집계하고, 각 SKU에 대해 `BookseenData`와 `KyoboData`를 개별 조회하여 `auto_select_distributor()`를 실행한 후 결과를 `VendorComparison`(결과 캐시 테이블)에 저장하고 반환한다.
...
### VendorComparison — 비교 결과 캐시 (v1.3.0 변경)

업로드 데이터를 직접 보유하지 않고 `run-comparison` 실행 결과만 저장한다.

```python
class VendorComparison(models.Model):
    """auto_select_distributor() 실행 결과 캐시. 업로드 데이터는 BookseenData/KyoboData에 저장된다."""
