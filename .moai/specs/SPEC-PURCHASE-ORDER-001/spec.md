...
**REQ-PO-033** (Event-Driven)
**When** 북센과 교보 양쪽의 업체 자료가 모두 업로드된 SKU에 대해, the 시스템 **shall** 다음 우선순위로 발주처를 자동 선택한다: (1) 재고 있는 업체 우선; (2) 양쪽 모두 재고 있는 경우 단가가 낮은 업체 선택.

**REQ-PO-034** (Ubiquitous)
...
The `업체 자료 업로드` 탭 **shall** 파일 업로드 영역(drag-and-drop 또는 파일 선택)과 유통사 선택 드롭다운(북센/교보)을 제공한다.

**REQ-PO-076** (Event-Driven)
**When** 업체 자료 업로드가 완료되면, the 시스템 **shall** 비교 결과 테이블을 표시하며 SKU별 북센 재고/단가, 교보 재고/단가, 자동 선택 발주처를 보여준다.

**REQ-PO-077** (Ubiquitous)
The 비교 결과 테이블 **shall** 관리자가 자동 선택된 발주처를 수동으로 변경할 수 있는 드롭다운을 각 행에 제공한다.
...
│   ├── 유통사 선택 드롭다운
│   ├── 파일 업로드 (drag-and-drop)
│   └── 비교 결과 테이블 (재고, 단가, 선택 발주처 드롭다운)
...
- 업체 파일 업로드 후 `comparison` 쿼리를 자동 무효화한다.
