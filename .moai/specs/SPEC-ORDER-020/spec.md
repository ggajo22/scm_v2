...
| 1.0.3 | 2026-08-14 | ggajo | 구현 완료(커밋 `b282d2d`). **아래 인용 줄 번호는 모두 구현 후 파일(총 481행) 기준으로 이 세션에서 직접 재확인했다** — 헬퍼 블록 삽입으로 `:51` 이후 줄 번호가 약 +50 밀렸으므로, v1.0.0~v1.0.2 및 `plan.md`가 인용한 줄 번호는 구현 이전 기준이라 현재 파일과 어긋난다. **구현 산출물**: `frontend/src/pages/LineItemNotesPage.tsx`의 평면 렌더링 블록을 그룹 렌더링(`:451`, `groupNotesByOrder(tabNotes).map(...)`)으로 교체. 신규 헬퍼 2개 — `compareByCreatedAtThenIdDesc`(`:70-78`), `groupNotesByOrder`(`:80-99`) — 와 `interface NoteOrderGroup`(`:58-62`) 추가. 단일 비교기를 그룹 정렬(`:96`)과 그룹 내부 정렬(`:92`) 양쪽에서 재사용해 tie-break이 두 층위에서 어긋날 수 없게 했다(REQ-GROUP-005/006). 그룹 컨테이너에 `data-testid="order-group-${orderId}"`(`:454`), 그룹 헤더에 `data-testid="order-group-header"`(`:458`) 부여. 헤더는 `order_name ?? \`주문 #${orderId}\``(`:461`)와 노트 건수만 표시하는 정보 전용이며 클릭 핸들러가 없다 — 폴백 규칙은 `NoteCard`의 주문번호 버튼(`:294`)과 동일. `@MX:NOTE` 2건 신규(`:51-53` 그룹화 입력이 `filterNotes` 출력임을 명시, `:64-69` 단일 비교기 재사용 근거 + 백엔드 `views.py:277-282`가 `-created_at` 단일 키로만 정렬한다는 tie-break 근거) — 본문은 `code_comments: en`에 따라 영어. 기존 `@MX:ANCHOR`(`:354`)와 `useLineItemNotes.ts`의 `@MX:WARN`은 검토 후 무변경. **무변경 확인**: `filterNotes`(`:35-49`), `NoteHistory`(`:104`), `InlineNoteForm`(`:153`), `NoteCard`(`:266`), `countByTab`(`:394`), `useLineItemNotes.ts`, `types/order.ts`, `backend/` 전량 — `git diff --stat`으로 백엔드·훅·타입 0행 확인, 프로덕션 diff는 hunk 2개(헬퍼 삽입, 렌더 블록 교체)뿐. **신규 테스트**: `frontend/src/pages/LineItemNotesPage.test.tsx`(`it` 블록 10개, T1~T10이 AC-GROUP-001~010에 1:1 대응). **검증(오케스트레이터가 직접 실행)**: 구현 전 기준선 — 페이지 테스트 3개 파일 14개 통과, 전체 스위트 189개. 구현 후 — 전체 23개 파일 199개 통과(`npx vitest run --config vitest.config.ts`). RED 확인 — `LineItemNotesPage.tsx`를 구현 이전 상태로 되돌리자 T1~T10 **10개 전부** 그룹 컨테이너 조회 단계에서 실패. Mutation 검증 — (M-a) 각 그룹이 자기 배열 대신 `tabNotes` 전체를 순회하게 바꾸자 AC-GROUP-005가 `expected [...] to have a length of 1 but got 2`로 실패(T1/T3/T4/T10도 부수 실패); (M-b) `id` tie-break을 제거해 비교기가 동률에서 `0`을 반환하게 하자 **정확히 T9/T10만** 실패하고 나머지 8개 통과; (M-d) 그룹 밖 툴바에 벌크 해결 버튼을 넣자 T1의 **페이지 스코프 절만** 실패(해결 컨트롤 총합 3→4)하고 T1의 그룹 스코프 절은 통과. **(M-c) `order_id`/`line_item_id` 내림차순 tie-break 오구현에서 T9/T10이 실패한다는 결과는 구현 에이전트 보고이며 오케스트레이터가 독립 재실행하지 않았다.** 각 mutation 후 파일을 백업본과 대조해 잔여물 0 확인. ESLint — 변경된 2개 파일 이슈 0(저장소의 기존 lint 에러는 무관·무수정). **발산 2건**: (1) `vi.mock` 팩토리가 `useLineItemNotes`의 6개 export를 모두 스텁으로 반환 — `plan.md`는 4개 생략이 이 SPEC 범위에서 안전하다고 했으나, 전부 반환해 리스크 R5를 후속 테스트 작성자에게까지 원천 제거. (2) 테스트 파일에 `as unknown as ReturnType<...>` 캐스트 2건 — `tsc --noEmit` 충족을 위한 것으로 `DashboardPage.test.tsx`의 기존 패턴과 동일. **미해결 검증 공백**: jsdom 렌더까지만 검증했고 **실제 브라우저 육안 확인은 하지 않았다** — 이 페이지는 인증이 필요한데 오케스트레이터가 자격 증명을 입력할 수 없다. **알려진 제약**: 주문번호가 그룹 헤더(`:461`)와 각 `NoteCard` 행(`:294`)에 두 번 표시된다 — `NoteCard` 무수정 제약(사용자 결정 1)의 절충이며 후속 과제. `groupNotesByOrder(tabNotes)`는 렌더마다 재계산되며 의도적으로 메모이제이션하지 않았다(엔드포인트가 페이지네이션 없이 전량을 반환하나 현재 규모에서 체감 차이가 없어 선제 최적화를 배제). |
