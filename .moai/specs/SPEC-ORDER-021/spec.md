...
| 1.5.0 | 2026-08-17 | ggajo | **AC-COST-009의 `ORDER_DETAIL_QUERY_COUNT` 상수를 7 → 8로 갱신** — acceptance.md DoD가 요구하는 "변경 시 상수를 실측값으로 갱신하고 그 사실을 spec.md HISTORY에 남긴다" 절차에 따른 기록이다. 원인은 이 SPEC이 아니라 주문상세/주문목록 표시 일원화(SPEC-ORDER-023 v1.5.0): `OrderDetailView`가 `line_items__purchase_orders` prefetch를 추가했다. 이 prefetch는 상세 응답에 새로 노출되는 `purchase_display`의 `_is_awaiting_purchase` 판정을 prefetch 캐시로 답하게 해, **라인아이템 수만큼의 쿼리를 상수 1개로 대체**한다 — AC-COST-009가 지키려는 불변식(쿼리 수가 라인아이템 수와 무관)은 그대로 유지되며, `orders_line_item`/`orders_exchangerate` 참조 쿼리 수도 각 1로 불변이다. `test_spec_021.py` 21개 전량 통과. |
