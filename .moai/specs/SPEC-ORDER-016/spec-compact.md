...
v1.0.2에서 요구사항을 39개에서 24개로 통합하고 연속 번호로 재부여했다(알파벳 접미사 폐지).
v1.0.4에서 강제 경로 행 잠금(REQ-FORCE-025)이 추가되어 001~025가 되었다.
...
- REQ-FORCE-025 — **한도 판정 직전에 지정 대상 LineItem을 요청 트랜잭션 안에서 행 단위로 잠그고,
  잠금 이후 읽은 값으로 REQ-FORCE-009/010을 판정**한다 → 동시 강제 요청 2건이 같은 낡은
  `shipped_quantity`로 각자 한도를 통과하는 경로 차단(설계 결정 O). **정상 경로는 무변경**
...
`spec.md` ACCEPTANCE CRITERIA 기준 총 23개(AC-FORCE-001~023). REQ 25개 전량이 최소 1개 AC에
...
- AC-FORCE-022 `[FE]` — 3행 중 2행 제출 시 네 결과: 제출한 2행 사라짐 / 미선택 1행 잔존 및 선택
  ...
  없음
- AC-FORCE-023 `[BE]` — `quantity=10`/`shipped_quantity=0` 대상에 각각 6을 요청하는 동시 실행 2건이
  둘 다 성공하지 않음: 하나만 반영(`shipped_quantity=6`)되고 다른 하나는 커밋된 값 기준으로
  `quantity_exceeded` 보고, 어떤 인터리빙에서도 `quantity` 초과 없음
...
- **정상** 출고 경로의 락 도입 없음 — 행 잠금은 강제 경로에만(REQ-FORCE-025, 설계 결정 O).
  `_process_outbound_rows`와 두 출고 엔드포인트는 무변경이며 두 경로의 관례 통일은 후속 과제 2
