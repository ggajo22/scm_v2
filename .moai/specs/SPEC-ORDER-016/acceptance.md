...
### AC-FORCE-023 — 동시 강제 실행 2건이 합산으로 한도를 넘기지 못함 `[BE]`

Traces: REQ-FORCE-025

- **Given**: `quantity=10`, `shipped_quantity=0`인 대상 L이 있다. 서로 다른 두 강제 요청 A와 B가
  각각 L을 대상으로 수량 6을 요청한다 — 단독으로는 각각 한도 내(`0+6 <= 10`)이지만 둘 다 반영되면
  `12 > 10`으로 한도를 넘긴다.
- **When**: A와 B를 각각 별도 DB 커넥션에서 동시에 실행하고, 두 요청이 **모두 한도 검사 이전 시점에
  도달한 뒤**에 진행되도록 동기화한다(구체 기법은 `plan.md` 참조).
- **Then**: 정확히 한 요청만 반영되어 L의 `shipped_quantity`가 `6`이 되고, 다른 요청은 먼저 커밋된
  값(`6`)을 기준으로 판정해 해당 대상을 `quantity_exceeded`로 보고한다. 어떤 실행 순서·인터리빙에서도
  L의 `shipped_quantity`는 `quantity`(10)를 넘지 않으며, 두 요청이 모두 성공하는 결과는 나오지
  않는다.
