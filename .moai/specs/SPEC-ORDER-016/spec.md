...
| 1.1.0 | 2026-08-17 | ggajo | **환불 차감 도입 — 강제 출고 경로의 용량도 잔여 수량 기준(REQ-FORCE-009/010).** `_process_force_outbound_rows`의 `effective_quantity`를 `max(quantity - Σrefunded, 0)`으로 정정해 `_process_outbound_rows`(SPEC-ORDER-015 v1.3.0)와 동일한 답을 내게 한다 — `_fully_refunded_line_item_ids`의 기존 주석이 경고하던 "한쪽만 차감하면 두 경로가 남은 출고 가능 수량에 대해 불일치"를 방지하기 위한 필수 동반 변경이다. 환불 합계는 `select_for_update()` SELECT에 주석 처리하지 않고 **별도 SELECT**로 조회한다 — MySQL은 FOR UPDATE의 잠금을 서브쿼리가 읽은 행까지 확장하므로, 이 경로가 쓰지 않는 `orders_refund` 행에 쓰기 잠금이 걸리는 것을 피하기 위함이다(신규 헬퍼 `_refunded_quantity_by_line_item`). 전량 환불 품목이 강제 출고 대상에서 제외되는 기존 게이트(REQ-FORCE-005)는 그대로다. **테스트**: `test_spec_016.py`에 `TestForceOutboundRefundNetting` 1건 추가, 동시성 마커 테스트 포함 전량 통과. |
