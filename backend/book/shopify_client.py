...
def _fetch_product_info(domain: str, token: str, product_id: str, variant_id: str) -> dict:
    """
    Single API call to GET /products/{product_id}.json — returns status, weight,
    price and image count. The product response includes variants with weight and
    price plus the images array, so no separate variant or image call is needed.
    ...
    try:
        ...
        # A missing images key means the response did not carry the data (null);
        # an empty list means the product genuinely has no images (0). The two are
        ...
        return {
            ...
            "image_count": len(images) if images is not None else None,
            "error": None,
        }
...
# @MX:NOTE: [AUTO] Single products API call returns status, variant weight, variant price
# and the product image count.
...
def fetch_store_live_info(
    ...
    """
    Fetch product status, weight, current selling price and image count for one store
    via a single API call.
    Returns dict with keys: status, weight, weight_unit, price, image_count, error.
    ...
    REQ-SHPINFO-016: image_count from the same response — no extra API call.
    """
