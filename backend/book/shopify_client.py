...
def _fetch_product_info(domain: str, token: str, product_id: str, variant_id: str) -> dict:
    """
    Single API call to GET /products/{product_id}.json — returns status, weight, price, image_count.
    The product response includes variants with weight/price and images array.
    ...
    if not product_id or product_id == "0":
        return {
            ...
            "price": None, "image_count": None, "error": "Invalid product_id",
        }
    try:
        ...
        return {
            "status": None, "weight": None, "weight_unit": None,
            "price": None, "image_count": None, "error": str(exc),
        }


# @MX:NOTE: [AUTO] Single products API call returns status, variant weight/price, and image count.
